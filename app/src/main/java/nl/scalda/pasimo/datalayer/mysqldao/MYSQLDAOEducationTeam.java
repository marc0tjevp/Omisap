...
public class MYSQLDAOEducationTeam implements IDAOEducationTeam {
	...
	public Set<EducationTeam> readAll() {
		...
		try {
			tx = session.beginTransaction();
			List educationTeamList = session.createNativeQuery("SELECT * FROM educationteam;").getResultList();
			for (Iterator iterator = educationTeamList.iterator(); iterator.hasNext();) {
				Object[] obj = (Object[]) iterator.next();
				EducationTeam et = new EducationTeam(String.valueOf(obj[2]), String.valueOf(obj[1]));
				teams.add(et);
			}
			tx.commit();
		} catch (Exception e) {
	...
	public void addTeacherToEducationTeam(Teacher teacher, EducationTeam educationTeam) {
		...
		try {
			...
			session.createNativeQuery(
					"INSERT INTO teacher_education_team (teacher_employeeNumber, education_team_id) VALUES (:teacheremployeeNumber, :educationTeamID);")
					.setParameter("teacheremployeeNumber", teacher.getEmployeeNumber())
					.setParameter("educationTeamID", educationTeam.getId()).executeUpdate();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
	...
	public void deleteTeacherFromEducationTeam(Teacher teacher, EducationTeam educationTeam) {
		...
		try {
			tx = session.beginTransaction();
			session.createNativeQuery(
					"DELETE FROM teacher_education_team WHERE teacher_employeeNumber = :employeeNumber")
